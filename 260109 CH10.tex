...
\begin{property}
    Soient $f:(G,*)\to(G',T)$ un morphisme de groupes et $e_G, e_{G'}$ les éléments neutres de $G$ et $G'$.
    \begin{enumerate}
        \item $f(e_G) = e_{G'}$.
...
\begin{proof}
    \begin{outline}
        \1 Preuve du premier point
            \2 On a $f(e_G) = f(e_G * e_G) = f(e_G) T f(e_G)$.
            \2 En ``simplifiant'' $f(e_G)$ des deux côtés, on obtient $f(e_G) = e_{G'}$.
