...
\MaPageDeGarde{Chapitre XIV-A : Algèbre Linéaire}{Retranscrit par Samy Youssoufine}{./assets/logo.png}{Peut contenir des erreurs ou des sections incomplètes.}
...
\begin{example}
    \begin{outline}
        ...
        \1 Soit $a = (a_1,\dots,a_n) \in \Kset^n$. On pose $H = \{x=(x_1,\dots,x_n) \in \Kset^n \mid \sum_{i=1}^{n} a_i x_i = 0\}$. Alors $H$ est un sous-ev. de $\Kset^n$, appelé l'hyperplan\index{hyperplan d'équation} de $\Kset^n$ d'équation $\sum_{i=1}^n a_i x_i = 0$.
...
\begin{property}
    ...
    \begin{enumerate}
        ...
        \item $B = (e_i)_{i \in \N^*}$ est une base de $E$ si et seulement si : 
        $$\forall x \in E \setminus \{0_E\}, \exists ! n \in \N^*, \exists ! (\lambda_1,\dots,\lambda_n) \in \Kset^n \tq \lambda_n \neq 0 \text{ et } x = \sum_{i=1}^{n}\lambda_i e_i$$
    \end{enumerate}
\end{property}

\begin{proof}
    \begin{itemize}
        \item \textbf{Démonstration de la première propriété :}
        \begin{itemize}
            \item[$\implies$] : Supposons que $B$ est une base de $E$. Par définition, $B$ est une famille génératrice, donc tout vecteur $x \in E$ admet au moins une décomposition $x = \sum_{k=1}^n \lambda_k e_k$. Supposons qu'il existe une autre décomposition $x = \sum_{k=1}^n \beta_k e_k$. Par soustraction, on obtient $\sum_{k=1}^n (\lambda_k - \beta_k) e_k = 0_E$. Comme $B$ est une base, elle est également libre. Sa liberté implique que tous les coefficients sont nuls : $\forall k \in \{1,\dots,n\}, \lambda_k - \beta_k = 0$, d'où $\lambda_k = \beta_k$. La décomposition est donc unique.
            \item[$\impliedby$] : L'existence d'une décomposition pour tout $x \in E$ assure immédiatement que $B$ est une famille génératrice de $E$. Pour la liberté, considérons $(\lambda_1,\dots,\lambda_n) \in \Kset^n$ tel que $\sum_{k=1}^n \lambda_k e_k = 0_E$. Or, on sait que $0_E = \sum_{k=1}^n 0 \cdot e_k$. Par l'hypothèse d'unicité de la décomposition du vecteur nul, on en déduit par identification que $\forall k \in \{1,\dots,n\}, \lambda_k = 0$. La famille $B$ est donc libre, ce qui en fait une base.
        \end{itemize}

        \item \textbf{Démonstration de la deuxième propriété :}
        \begin{itemize}
            \item[$\implies$] : Supposons que $B = (e_i)_{i \in \N^*}$ est une base de $E$. Soit $x \in E \setminus \{0_E\}$. Comme $B$ est génératrice, $x$ est combinaison linéaire d'un nombre fini de vecteurs de $B$. Il existe donc un indice maximal $n \in \N^*$ pour lequel le coefficient associé est non nul, d'où l'existence de la décomposition $x = \sum_{i=1}^n \lambda_i e_i$ avec $\lambda_n \neq 0$. 
            Pour l'unicité, supposons qu'il existe $m \in \N^*$ tel que $x = \sum_{i=1}^m \mu_i e_i$ avec $\mu_m \neq 0$. Supposons par l'absurde que $n > m$. On peut prolonger la seconde somme par des coefficients nuls jusqu'à $n$ ($\mu_i = 0$ pour $i > m$). Par soustraction, on obtient $\sum_{i=1}^n (\lambda_i - \mu_i) e_i = 0_E$. La famille $B$ étant une base, toute sous-famille finie est libre. Donc $\forall i \in \{1,\dots,n\}, \lambda_i = \mu_i$. Pour $i = n$, on aurait $\lambda_n = \mu_n = 0$, ce qui contredit l'hypothèse $\lambda_n \neq 0$. Par symétrie, on en déduit $n = m$. La liberté de la sous-famille libre $(e_1,\dots,e_n)$ impose ensuite l'unicité des scalaires $\lambda_i = \mu_i$ pour tout $i \in \{1,\dots,n\}$.
            \item[$\impliedby$] Supposons que tout vecteur de $E$ admette une unique décomposition sous forme de combinaison linéaire finie d'éléments de $B$.
            \begin{itemize}
                \item \textbf{Caractère générateur :} L'existence d'une telle famille de scalaires presque nulle pour tout vecteur $x \in E$ garantit par définition que $\text{Vect}(B) = E$, donc $B$ engendre $E$.
                
                \item \textbf{Liberté :} Soit $J \subset I$ une sous-famille finie d'indices et $(\alpha_j)_{j \in J}$ une famille de scalaires telle que :
                \[ \sum_{j \in J} \alpha_j e_j = 0_E \]
                On définit une famille de scalaires $(\lambda_i)_{i \in I}$ sur $I$ en posant $\lambda_i = \alpha_i$ si $i \in J$ et $\lambda_i = 0$ sinon. Cette famille est presque nulle par construction et vérifie $\sum_{i \in I} \lambda_i e_i = 0_E$.
                
                Or, le vecteur nul $0_E$ admet une décomposition triviale évidente où tous les scalaires sont nuls : $\sum_{i \in I} 0 \cdot e_i = 0_E$. Par l'hypothèse d'unicité de la décomposition appliquée au vecteur $0_E$, on identifie les coefficients terme à terme :
                \[ \forall i \in I, \quad \lambda_i = 0 \]
                En se restreignant aux indices de la sous-famille $J$, on obtient $\forall j \in J, \alpha_j = 0$. La famille $B$ est donc libre.
            \end{itemize}
            La famille $B$ étant libre et génératrice, elle constitue une base de $E$.
        \end{itemize}
    \end{itemize}
\end{proof}
...
\begin{attention}
    Il ne faut pas confondre ``supplémentaire'' et ``complémentaire''. En effet, si $F$ est un sev. de $E$, alors $C_E(F)$ (le complémentaire\index{complémentaire} de $F$ dans $E$) n'est pas un sev. car $0_E \notin C_E(F)$, alors $C_E(F)$ ne peut pas être un sev. supplémentaire de $F$ dans $E$.
\end{attention}
    ...
    \begin{outline}
        \1 Dans $E = \Kset^n$, on pose : $H = \{x = (x_1,\dots,x_n) \in \Kset^n \tq \sum_{i=1}^n x_i = 0\}$ et $D = \Kset \cdot \underbrace{(1,1,\dots,1)}_{=e}$. On a $\Kset^n = H \oplus D$.
            ...
            \2 Montrons maintenant que l'intersection de $H$ et $D$ est réduite à $\{0_E\}$.
                ...
                \3 Donc $H\cap D = \{0_E\}$.
    \end{outline}
    \label{exa:K^n_somme_directe_H_D}
\end{example}
...
\begin{exercise}
    ...
    On cherche un ensemble candidat $S$ tel que $\Kset(X) = \Kset[X] + S$ et $\Kset[X] \cap S = \{0_E\}$. On va donc le définir à partir de la recherche d'un élément $Y \in S$ tel que $P-Y \in \Kset[X]$ pour tout $P \in \Kset(X)$, ce qui nous rappelle facilement la DES de $P$, avec $P-Y$ la partie entière de $P$ et $Y$ la partie fractionnaire de $P$.
    
    Donc on peut poser $S = \{R \in \Kset(X) \tq R(0) = 0\}$. On montre que c'est un sev. de $\Kset(X)$, que $\Kset(X) = \Kset[X] + S$ et que $\Kset[X] \cap S = \{0_E\}$, ce qui nous permettra de conclure que $S$ est un sev. supplémentaire de $\Kset[X]$ dans $\Kset(X)$.

    \label{exo:supplementaires_divers}
\end{exercise}

\begin{remark}[Unicité du supplémentaire]
    Le supplémentaire n'est pas unique.

    En effet, en reprenant l'exemple \ref{exa:K^n_somme_directe_H_D}, on peut aussi montrer que $\Kset^n = H \oplus D'$, avec $D' = \Kset \cdot (1,0,\dots,0)$, et $D' \ne D$.
\end{remark}

\section{Somme d'une famille finie de sous-espaces vectoriels}

\begin{definition}[Somme d'une famille de sous-espaces vectoriels]
    Soient $E$ un $\Kset$-ev. et $F_1,\dots,F_n$ des sev. de $E$.

    \begin{enumerate}
        \item On définit la somme des $(F_k)_{1\le k \le n}$ par $\sum_{k=1}^n F_k = \{\sum_{k=1}^n x_k \tq x_k \in F_k\} \subset E$.
        \item La somme des $(F_k)_{1\le k \le n}$ est dite directe si $\forall (x_1,\dots,x_n) \in \prod_{k=1}^n F_k, \sum_{k=1}^n x_k = 0_E \implies \forall k, x_k = 0_E$.\\Dans ce cas, on note la somme par $\bigoplus_{k=1}^n F_k$.
    \end{enumerate}
\end{definition}
...
\begin{definition}
    ...
    (\textit{Il s'agit du cas particulier où la somme de $F_1,\dots,F_n$ est égale à $E$ et que cette somme est directe.})
\end{definition}
...
\begin{remark}
    $\forall 1 \le i \ne j \le n, F_i \cap F_j = \{0_E\}$ n'implique pas que $\sum_{k=1}^n F_k = \bigoplus_{k=1}^n F_k$.

    Par exemple, dans $E = \R^2$, on pose $F_1 = \R \cdot \underbrace{(1,0)}_{=e_1}$, $F_2 = \R \cdot \underbrace{(0,1)}_{=e_2}$ et $F_3 = \R \cdot \underbrace{(1,1)}_{=e}$.
...
\section{Dimension de la somme de sous-espaces vectoriels}
...
\begin{proof}
    ...
    Et donc $B = (e_{i,j})_{\substack{1 \le i \le n \\ 1 \le j \le n_i}}$ est une famille génératrice de $\bigoplus_{k=1}^n F_k$.
\end{proof}

\begin{remark}
    La base $B$ est appelée la base adaptée\index{base adaptée} à la somme directe.

    Et on écrit $B \overset{\text{noté}}= (B_1,\dots,B_n)$ pour montrer que $B$ est constituée de la réunion des bases $B_1,\dots,B_n$, au lieu de $B = \bigcup_{i=1}^n B_i$.
\end{remark}

\begin{attention}
    La notation $B = (B_1,\dots,B_n)$ ne signifie pas que $B$ est un $n$-uplet de bases, mais plutôt que $B$ est constituée de la réunion des bases $B_1,\dots,B_n$. C'est la notation de la \textbf{base adaptée} à la somme directe.
\end{attention}

\begin{consequence}
    Si $E$ est $\Kset$-ev. et $F_1,\dots,F_n$ sont des sev. de dimensions finies de $E$ tels que $E = \bigoplus_{k=1}^n F_k$,
    alors $E$ est de dimension finie et $\underset\Kset\dim(E) = \sum_{i=1}^n \underset\Kset\dim(F_i)$.
\end{consequence}
...
\begin{remark}
    Soient $F,G$ deux sous-espaces vectoriels de dimensions finies d'un $\Kset$-ev. $E$.

    Si $H$ est un supplémentaire de $F\cap G$ dans $F$, alors $H$ est un supplémentaire de $G$ dans $F+G$, i.e. : $F+G = H \oplus G$.
\end{remark}

\begin{consequence}
    Soient $E$ un $\Kset$-ev (de dimension finie) et $F,G$ des sous-espaces vectoriels de $E$ de dimension finie.
    ...
    \begin{enumerate}
        ...
        F\cap G = \{0_E\}\\
        \underset\Kset\dim(E) = \underset\Kset\dim(F) + \underset\Kset\dim(G)
        \end{cases}$
        \item $\displaystyle\begin{cases} E = F + G \\ \underset\Kset\dim(E) = \underset\Kset\dim(F) + \underset\Kset\dim(G)\end{cases}$
    \end{enumerate}

    \label{conseq:carac_somme_directe}
\end{consequence}
...
\begin{definition}[Hyperplan]
    ...
    On dit que $H$ est un hyperplan de $E$ lorsque $\exists a \in E\setminus\{0_E\}$ tel que $E = H \oplus \Kset \cdot a$.
\end{definition}

\begin{example}
    \begin{enumerate}
        \item $H = \{ x \in \Kset^n \tq \sum_{i=1}^n x_i = 0\}$ est un hyperplan de $\Kset^n$, sachant qu'on a déjà montré $\Kset^n = H \oplus \Kset \cdot e_1$.
        \item $\Kset_{n-1}[X]$ est un hyperplan de $\Kset_n[X]$, sachant que $\Kset_n[X] = \Kset_{n-1}[X] \oplus \Kset \cdot X^n$ (on peut le démontrer en utilisant la conséquence \ref{conseq:carac_somme_directe}).
        \item Soit $E =\mathcal{C}([a,b],\C)$. On pose $H = \{ f \in E \tqf \int_a^b f(x) dx = 0\}$ et $D = \{ f \in E \tqf f \text{ est constante}\}$. On a $D = \C\cdot\mathds{1}$ avec $\mathds{1} : \begin{matrix}
            [a,b] & \to & \C \\
            x & \mapsto & 1
        \end{matrix}$, et on a $E = H \oplus \C\cdot\mathds{1}$. En effet, on a déjà montré que $E = H \oplus D$ (exercice \ref{exo:supplementaires_divers}).
    \end{enumerate}
\end{example}
...
\begin{attention}
    Il faut toujours s'assurer que l'hyperplan ``candidat'' est bel et bien un sev. de $E$, avant de chercher à étudier sa dimension.

    Par exemple, $\R^{n-1}$ n'est pas un hyperplan de $\R^n$, même si $\underset\R\dim(\R^{n-1}) = n-1$, parce que $\R^{n-1}$ n'est pas un sev. de $\R^n$.
\end{attention}
