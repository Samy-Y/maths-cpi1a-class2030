...
\begin{proof}
    ...
    \begin{outline}
        ...
        \2 Soit $B_F$ une famille libre de $F$ telle que $\card{B_F} = d$. Montrons que $B_F$ est une base de $F$, i.e. $F = \Vect(B_F)$.
            ...
            \2 Donc $F$ est un $\Kset$-ev. de dimension finie, et $\underset\Kset\dim F = \card{B_F} = d \le n = \underset\Kset\dim E$.
            \2 Or $B_F$ est une base de $F$ alors $\underset\Kset\dim F = \card{B_F} = d \le n = \underset\Kset\dim E$.
...
\begin{exercise}
    ...
    \begin{align*}
        ...
        &= a_n(\underbrace{(X+1)^n - X^n}_{=\sum_{k=0}^{n-1} \legbinom{k}{n} X^k}) + \underbrace{\sum_{k=0}^{n-1} a_k ((X+1)^k - X^k)}_{\in \R_{n-1}[X]}\\
        &\in \R_{n-1}[X]
    \end{align*}
...
\begin{proof}
    ...
    Si $(V_1,\dots,V_p)$ est une famille génératrice de $F$, avec $\card{V_1,\dots,V_p} = p = \rang(V_1,\dots,V_p)$, alors $\Vect(V_1,\dots,V_p)$ est une base de $F$ (en particulier, elle est libre) et donc $\underset\Kset\dim F = \card{V_1,\dots,V_p} = p$, ce qui donne $\rang(V_1,\dots,V_p) = p$.
