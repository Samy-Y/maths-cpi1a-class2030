...
\begin{proof}
    ...
    \begin{outline}
        ...
        \1 Pour déterminer l'élément neutre de la multiplication $\times$, on doit trouver une suite $e = (e_n)_n$ dans $A^\N$ telle que pour toute suite $(x_n)_n$ dans $A^\N$, on ait $e \times (x_n)_n = (x_n)_n$ et $(x_n)_n \times e = (x_n)_n$.
            ...
            \2 On identifie donc $e$ à la suite $(\delta_{n,0})_n$.
...
\begin{property}[Conservation de l'intégrité]
    Si $A$ est un anneau intègre, alors $A^\N$ est aussi un anneau intègre.
\end{property}
...
\begin{proof}
    \begin{outline}
        \1 \textbf{Démonstration de l'existence (par récurrence forte sur $n = \deg(A)$) :}
            \2 Soit $B \in \Kset[X]\setminus\{0\}$ un polynôme fixé de degré $m \in \N$ et de coefficient dominant $b_m \neq 0$.
            
            \2 \textbf{Initialisation / Cas trivial ($\deg(A) < \deg(B)$) :}
                \3 Si $\deg(A) < m$, on pose simplement $Q = 0_{\Kset[X]}$ et $R = A$.
                \3 L'égalité $A = B \cdot 0_{\Kset[X]} + A$ est vérifiée de facto avec $\deg(R) = \deg(A) < \deg(B)$, ce qui valide l'existence.

            \2 \textbf{Hérédité / Cas général ($\deg(A) \ge \deg(B)$) :}
                \3 Supposons la propriété d'existence vérifiée pour tout polynôme de degré strictement inférieur à $n$ (avec $n \ge m$). 
                \3 Soit $A = \sum_{k=0}^{n}a_k X^k$ un polynôme de degré $n$, avec $a_n \ne 0$.
                \3 \textbf{Étape de réduction du degré :} On introduit le polynôme $A_1 = A - a_n b_m^{-1} X^{n-m} \cdot B$.
                \3 Par élimination mutuelle des termes dominants de $A$ et de ce produit, le coefficient de degré $n$ de $A_1$ s'annule, garantissant que $\deg(A_1) < n$.
                \3 \textbf{Application de l'hypothèse de récurrence :} On applique l'hypothèse forte à $A_1$. Il existe un couple $(Q_1, R) \in \Kset[X]^2$ tel que :
                   \[ A_1 = B \cdot Q_1 + R \quad \text{avec} \quad \deg(R) < \deg(B) \]
                \3 \textbf{Reconstruction de la division globale :} En substituant l'expression de $A_1$, on obtient :
                   \[ A - a_n b_m^{-1} X^{n-m} \cdot B = B \cdot Q_1 + R \]
                   \[ A = B \cdot \underbrace{\left(Q_1 + a_n b_m^{-1} X^{n-m}\right)}_{=Q} + R \]
                \3 Le couple $(Q,R)$ répond aux conditions au rang $n$, ce qui achève l'hérédité.

        \1 \textbf{Démonstration de l'unicité du couple $(Q,R)$ :}
            \2 Supposons l'existence de deux couples $(Q,R)$ et $(Q',R')$ vérifiant les conditions du théorème.
            \2 Par différence des deux égalités $A = BQ + R$ et $A = BQ' + R'$, on obtient par factorisation :
               \[ B \cdot (Q - Q') = R' - R \]
            \2 Supposons par l'absurde que $Q \neq Q'$. Alors $Q - Q' \neq 0_{\Kset[X]}$, ce qui donne sur les degrés :
               \[ \deg\left(B \cdot (Q - Q')\right) = \deg(B) + \deg(Q - Q') \ge \deg(B) \]
            \2 Or, par propriété des restes, la majoration de la différence impose :
               \[ \deg(R' - R) \le \max(\deg(R'), \deg(R)) < \deg(B) \]
            \2 On aboutit à la contradiction $\deg(B) \le \dots < \deg(B)$, ce qui impose $Q = Q'$.
            \2 En réinjectant $Q = Q'$ dans la relation de différence, il vient immédiatement $R = R'$.
    \end{outline}
\end{proof}
...
\begin{proof}
    ...
    \1 Si $x = x_k$ pour un certain $k \in [\![0,n]\!]$, alors $e(x_k) = f(x_k) - P(x_k) = 0$. De même, le produit $\prod_{k=0}^n (x_k - x_k)$ s'annule, ce qui valide immédiatement la formule.

    \1 Dans le cas où $x \ne x_k$ pour tout $k \in [\![0,n]\!]$, on introduit la fonction auxiliaire $g : [a,b] \to \Kset$ définie par :
    $$g(t) = f(t) - P(t) - e(x) \prod_{k=0}^{n} \frac{(t-x_k)}{(x-x_k)}$$

    \2 Cette fonction est de classe $\cclas{n+1}$ sur $[a,b]$ par combinaison de fonctions de classe $\cclas{n+1}$.

    \2 Par construction, pour tout $k \in [\![0,n]\!]$, on a $g(x_k) = 0$. De plus, comme $x$ est fixé, on a également $g(x) = f(x) - P(x) - e(x) = 0$. La fonction $g$ admet donc au moins $n+2$ racines distinctes dans $[a,b]$.

    \2 En appliquant le théorème de Rolle de manière itérée, la fonction dérivée première $g'$ s'annule au moins $n+1$ fois, $g''$ s'annule $n$ fois, et par récurrence, la dérivée d'ordre $n+1$ s'annule au moins une fois sur le segment.
    $$\exists \eps_x \in [a,b] \tq g^{(n+1)}(\eps_x) = 0$$

    \2 Calculons la dérivée d'ordre $n+1$ de $g$ par rapport à $t$ :
    \3 Le polynôme $P$ étant de degré au plus $n$, on a $P^{(n+1)}(t) = 0$.
    \3 Le produit $\prod_{k=0}^{n} (t-x_k)$ est un polynôme en $t$ de degré $n+1$ dont le coefficient dominant vaut $1$. Sa dérivée d'ordre $n+1$ par rapport à $t$ est donc la constante $(n+1)!$.
    \3 Les facteurs $e(x)$ et $\prod_{k=0}^{n} (x-x_k)$ sont constants par rapport à $t$.

    \2 On obtient ainsi pour tout $t \in [a,b]$ :
    $$g^{(n+1)}(t) = f^{(n+1)}(t) - e(x) \frac{(n+1)!}{\prod_{k=0}^{n} (x-x_k)}$$

    \2 En évaluant cette expression en $t = \eps_x$, on obtient :
    $$0 = f^{(n+1)}(\eps_x) - e(x) \frac{(n+1)!}{\prod_{k=0}^{n} (x-x_k)}$$

    \2 Ce qui donne, en isolant $e(x)$ :
    $$e(x) = \frac{f^{(n+1)}(\eps_x)}{(n+1)!} \prod_{k=0}^{n} (x-x_k)$$
    \end{outline}
\end{proof}
