...
\begin{remark}
    Par conséquent, on peut identifier $A$ à son image $f(A)$ dans $\Kset$. Et $f : A \to f(A)$ est un isomorphisme d'anneaux. \textbf{$A$ est donc isomorphe à $f(A)$}, qui est un sous-anneau de $\Kset$. Ainsi, on peut considérer $A$ comme un sous-anneau de $\Kset$, et on dit que $\Kset$ est un corps de fractions de $A$.
\end{remark}
...
\begin{proof}
    Nous allons montrer que, pour tout corps $L$ contenant $A$, $\Kset$ est forcément inclus dans $L$ ($\Kset \subseteq L$), ce qui implique que $\Kset$ est le plus petit corps contenant $A$.

    Soit $L$ un corps contenant $A$.

    \begin{outline}
        \1 Soit $\frac ND \in \Kset$ une fraction arbitraire avec $N \in A$ et $D \in A^*$. 
        \1 Comme $L$ est un corps contenant $A$, on a $N \in L$ et $D \in L^*$ (car $D$ est non nul dans $A$, il est aussi non nul dans $L$).
...
\begin{remark}[Valeur du degré d'une fraction rationnelle]
    ...
    $$\forall F \in \Kset(X), \deg(F) \in \Z\cup\{-\infty\}$$
    ...
    $$\forall F \in \Kset(X), \deg(F) = -\infty \iff F = 0_\Kset(X)$$
\end{remark}
...
\begin{remark}[Degré de la fraction dérivée]
    ...
    &\le \max\bigl((\deg(A) - 1) + \deg(B), \deg(A) + (\deg(B) - 1)\bigr) - 2\deg(B)\\
...
\begin{theorem}[Forme irréductible d'une fraction rationnelle]
    ...
    $$\forall F \in \Kset(X), \exists! (A,B) \in \Kset[X]^2, A \wedge B = 1, B \text{ unitaire et } F = \frac AB$$
\end{theorem}
...
\begin{proof}
    ...
    \begin{outline}
        ...
        \2 \textbf{On peut se permettre d'écrire $F = \frac{N_1}{D_1}$ car ils agissent comme d'autres représentants de cette classe d'équivalence.}
        \1 Montrons qu'on peut écrire $F$ sous la forme $\frac AB$ avec $A, B \in \Kset[X]$, $A \wedge B = 1$, $B$ unitaire et $\cfd(B) = 1$.
        \2 On a noté $F = \frac{N_1}{D_1}$ avec $N_1, D_1 \in \Kset[X]$ et $D_1$ unitaire.
        \2 Soit $C = N_1 \wedge D_1$.
        \2 Il existe un couple $(A,B) \in \Kset[X]^2$ tels que $A \wedge B = 1$ et $N_1 = CA$ et $D_1 = CB$.
        \2 Donc $F = \frac AB \avec A \wedge B = 1 \et \cfd(B) = 1$, sachant que $D_1$ et $C$ sont unitaires.
        \1 \textbf{Unicité} : Supposons que $F$ puisse s'écrire sous la forme $\frac AB$ et $\frac{A'}{B'}$ avec $A, B, A', B' \in \Kset[X]$, $A \wedge B = A' \wedge B' = 1$, $B$ et $B'$ unitaire. Alors, on a :
        \2 $A'B = AB'$ (en utilisant l'égalité $\frac AB = \frac{A'}{B'}$).
        \2 Comme $B \mid A B'$, alors en utilisant le théorème de Gauss (sachant que $A \wedge B = 1$), on trouve que $B \mid B'$. De la même manière, $B' \mid A' B$, donc $B' \mid B$.
...
\begin{definition}[Élément simple]
    ...
    \begin{outline}
        ...
        \2 $n \in \N^*$
        \1 tels que $F = \frac{A}{P^n}$, avec $\deg(A) < \deg(P)$ \faIcon{exclamation-triangle}.
    \end{outline}
\end{definition}
...
\begin{property}[Partie entière et partie fractionnaire]
    ...
    $$\forall F \in \Kset(X), \exists! (E, G) \in \Kset[X] \times \Kset(X), F = E + G \et \deg(G) < 0$$
\end{property}

\begin{proof}
    \textbf{On utilise simplement la division euclidienne de polynômes pour démontrer l'existence et l'unicité du couple $(E, G)$.}

    \begin{itemize}
        \item On pose $F = \frac AB \in \Kset(X)$ avec $A, B \in \Kset[X]$ et $B \neq 0_\Kset[X]$.

        \item On effectue la division euclidienne de $A$ par $B$, ce qui nous donne l'expression de $A$ suivante :
        $$A = EB + R$$
        avec $E, R \in \Kset[X]$ et $\deg(R) < \deg(B)$, où $E$ est le quotient (et agit comme la partie entière de $F$) et $R$ est le reste.

        \item On peut donc réecrire $F$ de la manière suivante :
        $$F = \frac{EB + R}{B} = E + \frac RB$$
        avec $E \in \Kset[X]$ et $\deg(\frac RB) = \deg(R) - \deg(B) < 0$ (car $\deg(R) < \deg(B)$).
    \end{itemize}
...
\begin{itemize}
    \item La première : Toute fraction $F$ qui peut s'écrire sous la forme $\frac{A}{B_1 \cdots B_n}$ avec $B_i$ et $B_j$ premiers entre eux pour tout $i \neq j$, peut se décomposer en une somme de fractions de la forme $\frac{A_i}{B_i}$, avec $\deg(A_i) < \deg(B_i)$.
    $$\forall F \in \Kset(X) \tqf F = \frac{A}{B_1 \cdots B_n} \et \forall i \neq j, B_i \wedge B_j = 1$$
    $$\exists! (A_1, \dots, A_n) \in \Kset[X]^n, F = \sum_{i=1}^{n}\frac{A_i}{B_i} \et \forall i, \deg(A_i) < \deg(B_i)$$
    \item La deuxième : Toute fraction $F$ qui peut s'écrire sous la forme $\frac{A}{B^n}$ avec $n \in \N^*$, peut se décomposer en une somme de fractions de la forme $\frac{A_k}{B^k}$, avec $\deg(A_k) < \deg(B)$.
    $$\forall F \in \Kset(X) \tqf F = \frac{A}{B^n} \et n \in \N^*, \exists! (A_1, \dots, A_n) \in \Kset[X]^n$$
    $$F = \sum_{k=1}^{n}\frac{A_k}{B^k} \et \forall k, \deg(A_k) < \deg(B)$$
\end{itemize}
...
\begin{proof}
    ...
    \begin{outline}
        \1 \textbf{Initialisation ($n=1$ et $n=2$) :}
            \2 Le cas $n=1$ est trivial. Établissons le cas critique $n=2$. Soit $F = \frac{A}{B_1 B_2}$ avec $\deg(A) < \deg(B_1 B_2)$.
            \2 \textbf{Existence pour $n=2$ :}
                \3 Puisque $B_1 \wedge B_2 = 1_A$, il existe un couple $(u,v) \in \Kset[X]^2$ tel que $u B_1 + v B_2 = 1_A$.
                \3 En injectant cette identité au numérateur, on obtient : $A = A(u B_1 + v B_2) = A v B_2 + A u B_1$.
                \3 Par division par le produit $B_1 B_2$, la fraction se sépare : $F = \frac{Av}{B_1} + \frac{Au}{B_2}$.
                \3 On ajuste les degrés par division euclidienne de $Av$ par $B_1$ et de $Au$ par $B_2$ :
                   \[ Av = Q_1 B_1 + A_1 \quad \text{avec} \quad \deg(A_1) < \deg(B_1) \]
                   \[ Au = Q_2 B_2 + A_2 \quad \text{avec} \quad \deg(A_2) < \deg(B_2) \]
                \3 En substituant, il vient : $F = (Q_1 + Q_2) + \frac{A_1}{B_1} + \frac{A_2}{B_2}$.
                \3 Comme $\deg(F) < 0$ et que la partie fractionnaire induite vérifie également $\deg\left(\frac{A_1}{B_1} + \frac{A_2}{B_2}\right) < 0$, l'unicité de la partie entière d'une fraction rationnelle impose $Q_1 + Q_2 = 0_A$, validant l'existence.
            \2 \textbf{Unicité pour $n=2$ :}
                \3 Supposons qu'il existe deux décompositions : $F = \frac{A_1}{B_1} + \frac{A_2}{B_2} = \frac{\tilde{A}_1}{B_1} + \frac{\tilde{A}_2}{B_2}$.
                \3 Par regroupement des termes de même dénominateur, on obtient : $\frac{A_1 - \tilde{A}_1}{B_1} = \frac{\tilde{A}_2 - A_2}{B_2}$.
                \3 L'égalité polynomiale associée s'écrit : $(A_1 - \tilde{A}_1)B_2 = (\tilde{A}_2 - A_2)B_1$.
                \3 $B_1$ divise le membre de droite, donc $B_1 \mid (A_1 - \tilde{A}_1)B_2$. Comme $B_1 \wedge B_2 = 1_A$, le théorème de Gauss implique que $B_1 \mid (A_1 - \tilde{A}_1)$.
                \3 Or, par hypothèse sur les degrés, $\deg(A_1 - \tilde{A}_1) < \deg(B_1)$. Le seul polynôme divisible par $B_1$ de degré strictement inférieur est le polynôme nul. On a donc $A_1 = \tilde{A}_1$, ce qui entraîne $A_2 = \tilde{A}_2$.
        
        \1 \textbf{Hérédité :}
            \2 Supposons la propriété vérifiée au rang $n$. Soit $F = \frac{A}{B_1 \cdots B_n B_{n+1}}$ avec $\deg(F) < 0$.
            \2 Posons le bloc de produit complémentaire $B' = B_2 \cdots B_{n+1}$. 
            \2 Les polynômes $(B_i)$ étant deux à deux premiers entre eux on a : $B_1 \wedge B' = 1_A$.
            \2 En appliquant le cas de base $n=2$ à la fraction $\frac{A}{B_1 B'}$, il existe un unique couple $(A_1, A')$ tel que :
               \[ F = \frac{A_1}{B_1} + \frac{A'}{B'} \quad \text{avec} \quad \deg(A_1) < \deg(B_1) \ \et \ \deg(A') < \deg(B') \]
            \2 Par hypothèse de récurrence appliquée à la fraction $\frac{A'}{B'} = \frac{A'}{B_2 \cdots B_{n+1}}$, il existe un unique $(n)$-uplet $(A_2, \dots, A_{n+1})$ tel que :
               \[ \frac{A'}{B'} = \sum_{i=2}^{n+1} \frac{A_i}{B_i} \quad \text{avec} \quad \forall i \in [\![2, n+1]\!], \ \deg(A_i) < \deg(B_i) \]
            \2 La substitution de la somme dans l'égalité de $F$ donne la décomposition attendue au rang $n+1$. L'unicité globale est garantie par l'unicité de chaque étape intermédiaire.
    \end{outline}
\end{proof}
...
\begin{example}
    ...
    \begin{outline}
        \1 Comme le degré de $F$ est négatif, on peut l'écrire sous la forme d'une somme de la forme $\frac{A_k}{B^k}$, puis multiplier par le dénominateur afin d'aboutir ensuite à un système de six équations à six inconnues, dont la résolution sera (évidemment) assez laborieuse.
        \1 Cependant, la deuxième méthode, plus rapide, est exactement analogue à celle qui a été utilisée dans la preuve.
        
        \2 On pose $A= X^5-3X^4+2X^2+2$, et on décompose $F = \frac{A_1}{(X^2+1)} + \frac{A_2}{(X^2+1)^2} + \frac{A_3}{(X^2+1)^3}$, avec $\deg(A_k) < 2$ pour $k \in \{1,2,3\}$.
        \2 On effectue ensuite la division euclidienne de $A$ par $X^2 + 1$ pour trouver un quotient $Q$ et un reste $R$ tels que $A = Q(X^2 + 1) + R$ avec $\deg(R) < 2$.
        \2 On trouve alors $Q = X^3 - 3X^2 - X + 5$ et $R=X-3$.
        \2 Par conséquent, $F = \frac{Q}{(X^2+1)^2} + \frac{R}{(X^2+1)^3}$.
        \2 Ensuite, on effectue la division euclidienne de $Q$ par $X^2 + 1$ pour trouver un quotient $Q_1$ et un reste $R_1$ tels que $Q = Q_1(X^2 + 1) + R_1$ avec $\deg(R_1) < 2$.
        \2 On trouve alors $Q_1 = X - 3$ et $R_1 = -2X + 8$.
        \2 Par conséquent, $F = \frac{Q_1}{(X^2+1)} + \frac{R_1}{(X^2+1)^2} + \frac{R}{(X^2+1)^3}$.
    \end{outline}
...
\subsection{Méthode par pôles doubles}
