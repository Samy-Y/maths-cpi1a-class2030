...
\begin{proof}
    ...
    On pose $\varphi : \begin{cases}I\to\Kset\\x\mapsto \begin{cases}
...
\begin{remark}
  \begin{enumerate}
    \item On a $\forall n \in \N^*, \mathcal{C}^{n-1}(I,\Kset) \subsetneq \mathcal{D}^n(I,\Kset) \subsetneq \mathcal{C}^n(I,\Kset)$ pour tout $n \in \N$.\\\textbf{Attention : il s'agit d'une inclusion stricte $\subsetneq \equiv \subset$ ! On note normalement les inclusions par $\subseteq$, mais dans ce cours, les inclusions non-strictes seront tout de même notées $\subset$}.
...
\begin{proof}
  ...
  Après quelques calculs de dérivées, on trouve que $g''(x) = f''(x) - A$.

  En remplaçant $x$ par $c$, on obtient $g''(c) = f''(c) - A = 0$, donc $A = f''(c)$.

  En remplaçant ensuite $A$ dans l'expression de $g(x)$, puis en introduisant $t$, on obtient la formule souhaitée.
\end{proof}
...
\begin{application}
  Soit $f:I\to\R$ dérivable sur $I$ telle que $f'$ est bornée sur $I$
  
  i.e. $\exists M\geq 0, \forall x\in I, |f'(x)| \leq M$. 
  
  Donc $f$ \textbf{est lipschitzienne} sur $I$, i.e. $\forall x,y\in I, |f(x)-f(y)| \leq M|x-y|$.
\end{application}
...
\begin{proof}
  \begin{outline}
    ...
    \2 Comme $f$ est croissante sur $I$, on a $f(y) - f(x) \ge 0$ si $y > x$ et $f(y) - f(x) \le 0$ si $y < x$.
    \2 Donc, $\frac{f(y) - f(x)}{y - x} \ge 0$.
    ...
    \2 Donc, comme $f'(c) \ge 0$, on a $f(y) - f(x) \ge 0$.
...
\begin{proof}
  \begin{itemize}
    \item On suppose que $f$ est strictement croissante sur $I$, mais que $A=\{x\in\inside{I},f'(x)=0\}$ est d'intérieur non-vide. C'est-à-dire qu'il existe $[a,b]\subset \inside{I}$ avec $a<b$ tel que $\forall x\in[a,b], f'(x) = 0$. Cela implique que $f$ est constante sur $[a,b]$, et donc que $f$ n'est pas strictement croissante sur $I$, ce qui contredit nos hypothèses. Ainsi, $A$ est d'intérieur vide.
...
\begin{theorem}[Inégalité des accroissements finis généralisée (IAF.G.)]
...
\begin{proof}
  Soient $x<y \in I$ (sans perte de généralité).
...
\begin{proof}
  On prend $g: I \mapsto M\cdot x$ et on applique l'inégalité des accroissements finis généralisée.

  \begin{itemize}
    \item On a $g'(x) = M$.
    \item Donc, $\abs{f'(x)} \leq M = g'(x)$.
    \item Donc, d'après l'inégalité des accroissements finis généralisée, $\forall x,y \in I, \abs{f(x)-f(y)} \leq \abs{g(x)-g(y)} = M\abs{x-y}$.
  \end{itemize}
...
\begin{consequence}
  ...
  $$\forall x,y \in \overset{\circ}{I}\tqp x<y, \abs{f(y)-f(x)} \leq \sup_{t\in[x,y]} |f'(t)| \cdot (y-x).$$
\end{consequence}
...
\begin{proof}
  ...
  On applique donc l'inégalité des accroissements finis généralisée.
\end{proof}
...
\begin{remark}
    \begin{enumerate}
        \item $f$ est dérivable en $b$ $\nimplies$ $\lim_{x\to b^-}f'(x)= f'(b)$ (Contre-exemple : $f:x\mapsto x^2\sin(\frac{1}{x})$ si $x\not=0$, $f(0)=0$).
...
\begin{exercise}
    ...
    Montrer que $f$ est de classe $\mathcal{C}^\infty$ sur $\R$.

    \textbf{Solution :}

    \begin{outline}
        \1 On a $f$ est de classe $\mathcal{C}^n$ sur $\R_+^*$ et $\R_-^*$.
        \1 On a $\lim_{x\to 0}f(x)=\lim_{x\to 0}e^{-\frac1{x^2}}=0=f(0)$.
        \1 Donc $f$ est continue en $0$.
        \1 \textbf{Nous allons montrer que $f^{(n)}$ admet une limite finie en 0.}
        \2 En faisant quelques dérivations, on remarque que :
        \2 $n=1, f'(x)=\frac{2}{x^3}e^{-\frac{1}{x^2}}$.
        \2 $n=2, f''(x)=\parens{\frac{4}{x^6}-\frac{6}{x^4}}e^{-\frac{1}{x^2}}$.
        \2 Nous allons donc montrer que $\forall n \ge 1, \exists P_n \in \R[X]$ tel que $\forall x \neq 0, f^{(n)}(x)=P_n(\frac{1}{x})e^{-\frac{1}{x^2}}$ avec $d^\circ P_n = 3n$.
        \3 Preuve par récurrence sur $n$.
        \3 Initialisation : $n=1$, $P_1(X)=2X^3$ avec $d^\circ P_1=3$.
        \3 Hérédité : Soit $n\in\N^*$. Supposons la propriété vraie au rang $n$.
        \3 (Calculs...)
        \3 On trouve $f^{(n+1)}(x)=\parens{\frac{-1}{x^2}P_n'(\frac1x)+\frac{2}{x^3}P_n(\frac1x)}e^{-\frac{1}{x^2}}$ avec $P_{n+1}(x)=\underbrace{2x^3P_n(x)}_{d^\circ = 3n+3}-\underbrace{x^2P_n'(x)}_{d^\circ=3n+1}$.
        \3 Donc $d^\circ P_{n+1}=3(n+1)$.
        \2 Donc on a $\forall n \ge 1, \forall x \neq 0, f^(n)(x)=P_n(\frac{1}{x})e^{-\frac{1}{x^2}}$.
        \2 Alors $\lim_{x\to0} f^{(n)}(x)=\lim_{t\to\infty}P_n(t)e^{-t^2}$.
        \1 Donc $\lim_{x\to0} f^{(n)}(x)=0 \in \R$, et ce, pour tout $n\ge 1$.
    \end{outline}
...
\begin{proof}
    ...
    \1 On pose $g:t\mapsto f(x)-\sum_{k=0}^n f^{(k)}(t)(x-t)^k-A(x-t)^{n+1}$ où $A$ est une constante telle que $g(a)=0$.

    \2 Donc $g$ est continue sur $I$ et dérivable sur $\inside{I}$, avec $g(x)=g(a)=0$ ($g$ étant définie pour $t$ dans $I$).

    \2 Donc, d'après le théorème de Rolle, il existe $c \in ]\min(a,x),\max(a,x)[$ tel que $g'(c)=0$.

    \2 Or nous savons que $g'(t)=-f^{(n+1)}(t)(x-t)^n - (n+1)A(x-t)^n$.

    \2 Donc $g'(c)=0 \iff A = \frac{f^{(n+1)}(c)}{(n+1)!}$.

    \1 En remplaçant $A$ dans l'expression de $g(a)=0$, on obtient la formule de Taylor-Lagrange.
    \end{outline}
\end{proof}
...
\begin{remark}[Changement de variable du roi]
    Soient $x,y \in I$ tels que $x<y$.

    On pose $\varphi : \begin{cases} [0,1]\to[x,y]\\t\mapsto tx+(1-t)y \end{cases}$

    On remarque que $\varphi$ est continue, dérivable et $\varphi' (t) = x-y < 0$.

    Donc $\varphi$ est bijective de $[0,1]$ vers $[x,y]$.

    $$\implies \begin{cases}
      \forall t \in [0,1], \varphi(t) \in [x,y]\\
      \forall a \in [x,y], \exists! t \in [0,1] : a = \varphi(t)
    \end{cases}$$

    Ce changement de variable est utile pour prouver des propriétés sur les fonctions convexes/concaves. Il est appelé \textbf{changement de variable du roi}. Il est aussi utilisé dans l'intégration ($I=\int_a^b f(x)dx = \int_a^b f(a+b-u)du$). Le \textbf{changement de variable de la reine} n'a pas été abordé en détail pendant le cours mais reste intéressant à étudier.
\end{remark}
...
\begin{property}
  ...
  $$f \text{ est convexe sur } I \iff \forall a \in I : \varphi_a : \begin{cases} I\setminus\{a\}\to\R \\ x\mapsto \frac{f(x)-f(a)}{x-a} \end{cases} \text{ est croissante.}$$
\end{property}

\begin{proof}
  \begin{outline}
    \1[$\rightarrow$] On commence la démonstration dans le sens direct.
    \1 Il faudra distinguer entre trois cas, sachant qu'il y a trois paramètres, donc trois positions relatives possibles.
    \1 Les cas à distinguer seront donc : $x<a<y$, $a<x<y$, et $x<y<a$.
    \1 Dans ce cours, nous ne traiterons que le cas $x<a<y$, les deux autres étant similaires.
    \2 Soient $x,y \in I$ tels que $x<a<y$.
    \2 Donc $a \in ]x,y[$.
    \2 Donc $\exists t \in ]0,1[ : a = tx + (1-t)y$.
    \2 Donc $\exists t \in ]0,1[ : t\cdot f(a)+(1-t)\cdot f(a)=f(a)$.
    \2 Donc $\exists t \in ]0,1[ : t\cdot f(a)+(1-t)\cdot f(a)=f(tx+(1-t)y)$.
    \2 Par convexité de $f$, on a donc $\exists t \in ]0,1[ : t\cdot f(a)+(1-t)\cdot f(a)\le tf(x)+(1-t)f(y)$.
    \2 Donc $t(f(a)-f(x))\le (1-t)(f(y)-f(a))$.
    \2 Donc $f(a)-f(x)\le \frac{1-t}{t}(f(y)-f(a))$.
    \2 On a $a-x=\dots=(1-t)(y-x)$ et $y-a=t(y-x)$.
    \2 Donc $\frac{1-t}{t}=\frac{a-x}{y-a}$.
    \2 Donc $f(a)-f(x)\le \frac{a-x}{y-a}(f(y)-f(a))$.
    \2 Donc $\frac{f(a)-f(x)}{a-x}\le \frac{f(y)-f(a)}{y-a}$.
    \2 Donc $\varphi_a(x)\le \varphi_a(y)$.
    \2 Donc, $\varphi_a$ est croissante.
    \1[$\leftarrow$] On commence la démonstration dans le sens réciproque.
    \1 On suppose que $\forall a \in I, \varphi_a$ est croissante.
    \2 Soient $x,y \in I, t \in [0,1]$ tels que $x\not=y$.
    \2 On prendra par exemple $x<y$ sans perte de généralité.
    \2 On pose $a = tx + (1-t)y \in ]x,y[$.
    \2 Donc, par hypothèse, on a $\varphi_a(x) \le \varphi_a(y)$.
    \2 Donc, $\frac{f(a)-f(x)}{a-x} \le \frac{f(y)-f(a)}{y-a}$.
    \2 Donc $f(a)-f(x) \le \underbrace{\frac{a-x}{y-a}}_{=\frac{1-t}{t}}(f(y)-f(a))$ (quelques cas particuliers $t=0\dots$ ont été omis pour simplifier la démonstration).
    \2 Donc $t(f(a)-f(x)) \le (1-t)(f(y)-f(a))$.
    \2 Donc $tf(a)+(1-t)f(a) \le tf(x)+(1-t)f(y)$.
    \2 On en déduit donc que $f$ est convexe sur $I$.
    \1 Ainsi, on a montré l'équivalence.
  \end{outline}
\end{proof}

\begin{property}
  Soit $f \in \R^I$ une fonction convexe.

  $\forall a<b<c \in I, f \text{ est dérivable à droite et à gauche en } b \text{ avec } \frac{f(b)-f(a)}{b-a} \le f'_g(b) \le f'_d(b) \le \frac{f(c)-f(b)}{c-b}$
\end{property}

\begin{proof}
  \begin{itemize}
    \item On pose $\psi_1:\begin{cases} ]a,b[\to\R \\ u\mapsto \frac{f(u)-f(b)}{u-b} \end{cases}$.
    \item On pose $\psi_2:\begin{cases} ]b,c[\to\R \\ v\mapsto \frac{f(v)-f(b)}{v-b} \end{cases}$.
    \item On peut aussi considérer une fonction $\varphi : \begin{cases} I\setminus\{b\}\to\R \\ x\mapsto \frac{f(x)-f(b)}{x-b} \end{cases}$, puis considérer ses restrictions à $]a,b[$ et $]b,c[$.
    \item On sait que $\psi_1$ est croissante.
    \item On peut donc montrer que $\psi_1$ est majorée et qu'elle admet donc une limite réelle et finie en $b$.
    \item En réalité, $\psi_1$ est majorée par $\varphi_b(v)$, en fixant $v\in]b,c[$.
    \item Donc, $\lim_{u\to b^-}\psi_1(u)$ existe et est finie. $\psi_1$ admet une limite finie à gauche en $b$.
    \item On a donc $\psi_1(a) \le \lim_{u\to b^-}\psi_1(u) \le \psi_1(v)$.
    \item Donc $f$ est dérivable à gauche avec $\frac{f(b)-f(a)}{b-a} \le \lim_{x \to b^-}\frac{f(x)-f(b)}{x-b} \le \frac{f(v)-f(b)}{v-b}$.
    \item Donc $\forall v \in ]b,c[, \frac{f(b)-f(a)}{b-a} \le f'_g(b) \le \psi_2(v)$.
    \item On sait aussi que $\psi_2$ est croissante.
    \item Elle est minorée par $f'_g(b)$. Donc $\psi_2$ admet une limite finie à droite en $b$.
    \item Donc, $\forall u \in ]a,b[, f'_g(b) \le f'_d(b) \le \frac{f(c)-f(b)}{c-b}$.
    \item Ainsi, on a montré que $f$ est dérivable à droite et à gauche en $b$ avec $\frac{f(b)-f(a)}{b-a} \le f'_g(b) \le f'_d(b) \le \frac{f(c)-f(b)}{c-b}$.
  \end{itemize}
\end{proof}

\begin{consequence}
  Soit $f \in \R^I$ une fonction convexe sur $I$.

  Alors, $f$ est continue sur $\overset{\circ}{I}$.
\end{consequence}
...
\begin{consequence}
  ...
  $$f \text{ est convexe sur } I \iff f'' \ge 0 \text{ sur } \overset{\circ}{I}$$
\end{consequence}

\begin{property}
  Soit $f \in \mathcal{D}^1(I,\R)$.

  $$f \text{ est convexe sur } I \iff C_f \text{ est au-dessus de ses tangentes sur } I$$
\end{property}
...
\begin{tikzpicture}[scale=1.2]
  ...
  % Fonction convexe f(x) = 0.4x^2 + 0.5
  \draw[thick, blue, domain=-3.2:3.2, samples=100] plot (\x, {0.4*\x*\x + 0.5});
  \node[blue, right] at (2.3, 4) {$C_f$};
  
  % Tangentes en plusieurs points
  % Tangente en x = -2.5 : y = f(-2.5) + f'(-2.5)(x - (-2.5)) = 3 - 2(x + 2.5)
  \draw[red!80!black, thin, domain=-3.5:0, samples=50] plot (\x, {3 + (-2)*(\x + 2.5)});
  
  % Tangente en x = -1.8 : f(-1.8) = 1.796, f'(-1.8) = -1.44
  \draw[red!70!black, thin, domain=-3.2:0.8, samples=50] plot (\x, {1.796 + (-1.44)*(\x + 1.8)});
  
  % Tangente en x = -1 : f(-1) = 0.9, f'(-1) = -0.8
  \draw[red!60!black, thin, domain=-2.8:1.5, samples=50] plot (\x, {0.9 + (-0.8)*(\x + 1)});
  
  % Tangente en x = -0.3 : f(-0.3) = 0.536, f'(-0.3) = -0.24
  \draw[orange!80!black, thin, domain=-2.5:2.2, samples=50] plot (\x, {0.536 + (-0.24)*(\x + 0.3)});
  
  % Tangente en x = 0 : f(0) = 0.5, f'(0) = 0
  \draw[orange, thin, domain=-3:3, samples=50] plot (\x, {0.5});
  
  % Tangente en x = 0.4 : f(0.4) = 0.564, f'(0.4) = 0.32
  \draw[orange!80!black, thin, domain=-2.2:2.8, samples=50] plot (\x, {0.564 + 0.32*(\x - 0.4)});
  
  % Tangente en x = 1 : f(1) = 0.9, f'(1) = 0.8
  \draw[red!60!black, thin, domain=-1.5:2.8, samples=50] plot (\x, {0.9 + 0.8*(\x - 1)});
  
  % Tangente en x = 1.8 : f(1.8) = 1.796, f'(1.8) = 1.44
  \draw[red!70!black, thin, domain=-0.8:3.2, samples=50] plot (\x, {1.796 + 1.44*(\x - 1.8)});
  
  % Tangente en x = 2.5 : f(2.5) = 3, f'(2.5) = 2
  \draw[red!80!black, thin, domain=0:3.5, samples=50] plot (\x, {3 + 2*(\x - 2.5)});
  
  % Points de tangence
  \foreach \a in {-2.5, -1.8, -1, -0.3, 0, 0.4, 1, 1.8, 2.5} {
    \fill[black] (\a, {0.4*\a*\a + 0.5}) circle (1.5pt);
  }
  
  % Légende
  \node[red!70!black, right] at (1.5, -0.3) {\small Tangentes $(T_a)$};
\end{tikzpicture}
\end{center}

\begin{remark}
  L'équation de la tangente à la courbe représentative de $f$ au point d'abscisse $a$ est donnée par : $(T_a):y = f(a) + f'(a)(x-a)$.
\end{remark}

\begin{proof}
  On cherche à démontrer que pour tout $a \in I$, et que pour tout $x \in I$, on a $f(x) \ge f(a) + f'(a)(x-a)$.

  \textbf{Démonstration dans le sens direct :}

  \begin{itemize}
    \item Soit $a \in I$.
    \item Soit $x \in I$, avec $x \ne a$.
    \item On pose $\varphi_a : \begin{cases} I\setminus\{a\}\to\R \\ t\mapsto \frac{f(t)-f(a)}{t-a} \end{cases}$ croissante (car $f$ est convexe).
    \item Donc, si $x<a$, on a $\varphi_a(x) \le \lim_{t\to a^-}\varphi_a(t) = f'_g(a) \le f'_d(a)$.
    \item Donc, si $x<a$, on a $\frac{f(x)-f(a)}{x-a} \le f_g'(a)$ et $\frac{f(x)-f(a)}{x-a} \le f_d'(a)$.
    \item Donc, si $x<a$, on a $f(x) - f(a) \ge f_g'(a)(x-a)$ et $f(x) - f(a) \ge f_d'(a)(x-a)$.
    \item Donc, si $x<a$, on a $f(x) \ge f(a) + f_g'(a)(x-a)$ et $f(x) \ge f(a) + f_d'(a)(x-a)$ (1). On en déduit que la courbe est au-dessus de la demi-tangente à droite et à gauche en $a$ pour $x<a$.
    \item On répète le raisonnement pour le cas $x>a$.
    \item Ainsi, on a montré que pour tout $a \in I$, et que pour tout $x \in I$, la courbe de $f$ est au-dessus de ses demi-tangentes en $a$.
  \end{itemize}

  \textbf{Démonstration dans le sens réciproque à refaire.}
