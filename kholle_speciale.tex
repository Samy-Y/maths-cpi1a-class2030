...
\begin{itemize}
    \item[\textbf{1)}] Puisque $E$ est de dimension finie, il admet une famille génératrice finie $G$. La famille $L_0 \cup G$ est alors trivialement génératrice de $E$.
...
\begin{rappel}
	Toutes les bases d'un espace vectoriel de dimension finie ont le même cardinal, égal à la dimension de l'espace. En effet, pour deux bases arbitraires $\mathcal{B}_1$ et $\mathcal{B}_2$, on a $\mathrm{Card}(\mathcal{B}_1) \le \mathrm{Card}(\mathcal{B}_2)$ et $\mathrm{Card}(\mathcal{B}_2) \le \mathrm{Card}(\mathcal{B}_1)$, d'où l'égalité des cardinaux.
\end{rappel}
...
\begin{itemize}
	...
	\item ($\implies$) Dans le sens direct, on applique le théorème de la base incomplète à la famille libre $L$ et à la famille génératrice $G$ pour construire une base $B$ de $E$ telle que $L \subset B \subset G$. Comme $\mathrm{Card}(L) = n$ et $\mathrm{Card}(B) = n$, alors $L = B$, donc $L$ est une base de $E$. De même, comme $\mathrm{Card}(G) = n$ et $\mathrm{Card}(B) = n$, alors $G = B$, donc $G$ est une base de $E$.
\end{itemize}
...
\begin{remarque}
La définition d'une somme directe de $F_k$ énonce que :
\[
...
\begin{tcolorbox}[instructionstyle]
	\textbf{Formule de Grassmann :} Soit $E$ un $\mathbb{K}$-espace vectoriel de dimension finie, et $F$ et $G$ deux sev de $E$. Montrer que :
	\[\underset{\mathbb{K}}{\dim}(F + G) = \underset{\mathbb{K}}{\dim}(F) + \underset{\mathbb{K}}{\dim}(G) - \underset{\mathbb{K}}{\dim}(F \cap G)\]
\end{tcolorbox}

\begin{rappel}
	Il faut savoir démontrer que tout sous-espace vectoriel $F$ d'un espace vectoriel $E$ de dimension finie admet un supplémentaire dans $E$, c'est-à-dire qu'il existe un sous-espace vectoriel $G$ de $E$ tel que $E = F \oplus G$. Il est aussi intéressant de pouvoir redémontrer $\dim(F \oplus G) = \dim(F) + \dim(G)$ à partir de la définition d'une somme directe.
\end{rappel}
...
\begin{itemize}
	\item On sait que $F$ est de dimension finie, notée $p$.
	\item Considérons $B_1 = (e_1, \dots, e_p)$ une base de $F$.
	\item Comme $B_1$ est aussi une famille libre de $E$, on peut la compléter en une base $B = (e_1, \dots, e_p, e_{p+1}, \dots, e_n)$ de $E$, où $n$ est la dimension de $E$.
	\item On pose $H = \text{Vect}(e_{p+1}, \dots, e_n)$. Par construction, $H$ est un supplémentaire de $F$ dans $E$, c'est-à-dire que $E = F \oplus H$.
	\item En effet, pour tout $x \in E$, on peut écrire $x = \sum_{i=1}^p \alpha_i e_i + \sum_{j=p+1}^n \beta_j e_j$, avec $\sum_{i=1}^p \alpha_i e_i \in F$ et $\sum_{j=p+1}^n \beta_j e_j \in H$. De plus, l'intersection $F \cap H = \{0_E\}$, car les vecteurs de $B_1$ et de $H$ sont linéairement indépendants.
\end{itemize}
Montrons que $F+G$ est de dimension finie et que $F+G = H \oplus G$.
\begin{itemize}
	\item $F+G$ est de dimension finie, car $F$ et $G$ sont de dimension finie. En effet, on peut poser $F = \text{Vect}(B_1)$ et $G = \text{Vect}(B_2)$, où $B_1$ et $B_2$ sont des bases de $F$ et $G$ respectivement. Alors, $F+G = \text{Vect}(B_1 \cup B_2)$, qui est de dimension finie.
	\item $F \cap G$ est aussi un sous-espace vectoriel de $E$, de dimension finie.
	\item Alors, d'après ce qui précède, $F \cap G$ admet un supplémentaire $H$ dans $F$, c'est-à-dire que $F = (F \cap G) \oplus H$.
\end{itemize}
Montrons que $F+G = H \oplus G$.
\begin{itemize}
	\item Comme $H \subset F$, l'intersection $G \cap H = \{0_E\}$, car $G \cap H = G \cap (F \cap H) = H \cap (F \cap G)$ et $H \cap (F \cap G) = \{0_E\}$.
	\item Soit $x \in F+G$. Alors, $x = f + g$ avec $f \in F$ et $g \in G$. Comme $F = (F \cap G) \oplus H$, on peut écrire $f = h + k$ avec $h \in H$ et $k \in F \cap G$. Ainsi, $x = h + k + g$. Comme $k + g \in G$, on a $x = h + (k + g)$ avec $h \in H$ et $(k + g) \in G$. De plus, l'intersection $H \cap G = \{0_E\}$, donc la décomposition est unique. Ainsi, $F+G = H \oplus G$.
\end{itemize}
...
\begin{tcolorbox}[instructionstyle]
	\textbf{Théorème du rang :} Soit $E$ et $F$ deux $\mathbb{K}$-espaces vectoriels avec $\underset{\mathbb{K}}{\dim}(E) = n < +\infty$. Montrer que pour toute application linéaire $f \in \mathcal{L}(E, F)$, on a :
	\[\underset{\mathbb{K}}{\dim}(\ker(f)) + \underset{\mathbb{K}}{\dim}(\mathrm{Im}(f)) = n\]
\end{tcolorbox}

\begin{rappel}
	Il faut savoir démontrer que, si $H$ est un supplémentaire de $\ker(f)$ dans $E$, i.e. $E = \ker(f) \oplus H$, alors $H$ est isomorphe à $\mathrm{Im}(f)$, i.e. $H \sim \mathrm{Im}(f)$.
\end{rappel}

\begin{tcolorbox}[methodstyle]
	On considère d'abord $H$ un supplémentaire de $\ker(f)$ dans $E$, c'est-à-dire que $E = \ker(f) \oplus H$. On montre ensuite que la restriction de $f$ à $H$ est injective et que son image est exactement $\mathrm{Im}(f)$. Ainsi, on obtient un isomorphisme entre $H$ et $\mathrm{Im}(f)$, ce qui permet de conclure que $\dim(H) = \dim(\mathrm{Im}(f))$. Enfin, on utilise la formule de la dimension pour obtenir le théorème du rang.
\end{tcolorbox}

Soit $H$ un supplémentaire de $\ker(f)$ dans $E$, c'est-à-dire que $E = \ker(f) \oplus H$. On considère la restriction de $f$ à $H$, notée $f|_H : H \to F$.
$$f|_H(x) = f(x) \quad \forall x \in H$$
Pour tout $x \in H$, on a bien $f|_H(x) \in \mathrm{Im}(f)$, donc $f|_H$ est une application linéaire de $H$ dans $\mathrm{Im}(f)$, i.e. :
$$f|_H \in \mathcal{L}(H, \mathrm{Im}(f))$$
...
Montrons maintenant que $f|_H$ est surjective sur $\mathrm{Im}(f)$. On a $E = H \oplus \ker(f)$, donc $f(E) = f(H) + f(\ker(f)) = f(H) + \{0_F\} = f(H)$. Ainsi, $\mathrm{Im}(f) = f(H)$, ce qui montre que $f|_H$ est surjective sur $\mathrm{Im}(f)$.

Ainsi, $f|_H$ est un isomorphisme entre $H$ et $\mathrm{Im}(f)$, ce qui implique que $\dim(H) = \dim(\mathrm{Im}(f))$. Enfin, on utilise la formule de la dimension pour obtenir :
\[\dim(E) = \dim(\ker(f)) + \dim(H) = \dim(\ker(f)) + \dim(\mathrm{Im}(f))\]

\clearpage

\begin{tcolorbox}[instructionstyle]
	Montrer que, si $f \in \mathcal L(E)$ est un projecteur, alors $E = \ker(f) \oplus \mathrm{Im}(f)$.
\end{tcolorbox}

\begin{outline}
	\1 On montre que l'intersection de $\mathrm{Im}(f)$ et $\ker(f)$ est réduite au vecteur nul.
	\2 On prend $x \in \mathrm{Im}(f) \cap \ker(f)$, alors $\exists y \in E$ tel que $x = f(y)$, et $f(x) = 0_E$, donc $f(f(y)) = 0_E$, donc $f(y) = 0_E$, donc $x = f(y) = 0_E$.
	\1 On montre que $E = \mathrm{Im}(f) + \ker(f)$.
	\2 On remarque directement que $\forall x \in E, f^2(x) = f(x) \implies f(f(x)) = f(x) \implies f(f(x)-x)=0_E \implies f(x)-x \in \ker(f)$. Donc $x = f(x) + (x - f(x))$ avec $f(x) \in \mathrm{Im}(f)$ et $x - f(x) \in \ker(f)$, et par conséquent, $E = \mathrm{Im}(f) + \ker(f)$.
	\1 Donc $E = \mathrm{Im}(f) \oplus \ker(f)$.
\end{outline}
...
\begin{tcolorbox}[instructionstyle]
	Montrer que $\forall f \in \mathcal L(E)$, si $f$ est une involution, alors $E = \ker(f - \mathrm{Id}_E) \oplus \ker(f + \mathrm{Id}_E)$.
\end{tcolorbox}

\begin{rappel}
	Une involution est un endomorphisme $f$ tel que $f^2 = \mathrm{Id}_E$. Toute symétrie par rapport à $F$ parallèlement à $G$ où $E = F \oplus G$, c'est-à-dire $\forall x \in F, s(x) = x$ et $\forall y \in G, s(y) = -y$, est une involution. La propriété que nous allons démontrer nous permettra aussi d'établir la réciproque : toute involution est une symétrie par rapport à $\ker(f - \mathrm{Id}_E)$ parallèlement à $\ker(f + \mathrm{Id}_E)$.
\end{rappel}

\begin{tcolorbox}[methodstyle]
	En premier, on montre que l'intersection $\ker(f - \mathrm{Id}_E) \cap \ker(f + \mathrm{Id}_E) = \{0_E\}$ en utilisant la définition d'une involution. Ensuite, on procède par analyse-synthèse pour montrer que tout vecteur $x \in E$ peut s'écrire de manière unique comme la somme d'un vecteur de $\ker(f - \mathrm{Id}_E)$ et d'un vecteur de $\ker(f + \mathrm{Id}_E)$. On décomposera le vecteur $x$ sous la forme $x = \frac{1}{2}(x + f(x)) + \frac{1}{2}(x - f(x))$ et on montrera que les deux termes appartiennent respectivement à $\ker(f - \mathrm{Id}_E)$ et $\ker(f + \mathrm{Id}_E)$.
\end{tcolorbox}

\begin{outline}
	\1 Montrons que $\ker(f - \mathrm{Id}_E) \cap \ker(f + \mathrm{Id}_E) = \{0_E\}$.
	\2 Soit $x \in \ker(f - \mathrm{Id}_E) \cap \ker(f + \mathrm{Id}_E)$, alors $f(x) = x$ et $f(x) = -x$, donc $x = 0_E$, et par conséquent, $\ker(f - \mathrm{Id}_E) \cap \ker(f + \mathrm{Id}_E) = \{0_E\}$.
	\1 Montrons que $E = \ker(f - \mathrm{Id}_E) + \ker(f + \mathrm{Id}_E)$.
	\2 Soit $x \in E$. On va procéder par analyse-synthèse.
	\2 \textit{Analyse :}
	\3 On suppose que $x = x_1 + x_2$ avec $x_1 \in \ker(f - \mathrm{Id}_E)$ et $x_2 \in \ker(f + \mathrm{Id}_E)$.
	...
	\3 Donc $x_1 \in \ker(f - \mathrm{Id}_E)$ et $x_2 \in \ker(f + \mathrm{Id}_E)$.
	\3 Alors $x = x_1 + x_2$.
	\1 Donc $E = \ker(f - \mathrm{Id}_E) \oplus \ker(f + \mathrm{Id}_E)$.
\end{outline}

\clearpage

\begin{tcolorbox}[instructionstyle]
	\textbf{Lemme des noyaux :} Soit $u \in \mathcal L(E)$. Soient $P$ et $Q$ deux polynômes de $\mathbb K[X]$ premiers entre eux. Montrer que :
	$$\ker((PQ)(u)) = \ker(P(u)) \oplus \ker(Q(u))$$
\end{tcolorbox}

\begin{tcolorbox}[methodstyle]
	On utilise le théorème de Bézout pour écrire $UP + VQ = 1$ pour certains polynômes $U, V \in \mathbb K[X]$. On obtient donc une identité d'endomorphismes $U(u)P(u) + V(u)Q(u) = \mathrm{Id}_E$. Cette identité est ensuite utilisée pour démontrer que $\ker(P(u)) + \ker(Q(u)) = \ker((PQ)(u))$ en utilisant une double-inclusion. Enfin, on montre que l'intersection $\ker(P(u)) \cap \ker(Q(u)) = \{0_E\}$ en évaluant l'identité précédente sur $x \in \ker(P(u)) \cap \ker(Q(u))$, et donc que la somme est directe, ce qui nous permet de conclure.
\end{tcolorbox}

Puisque $P$ et $Q$ sont premiers entre eux ($\mathrm{pgcd}(P,Q) = 1$), il existe $U,V \in \mathbb K[X]$ tels que $UP + VQ = 1$. En évaluant en $u$, on obtient l'identité d'endomorphismes :
$$U(u)P(u) + V(u)Q(u) = \mathrm{Id}_E$$

Pour montrer que $\ker(P(u)) + \ker(Q(u)) = \ker((PQ)(u))$, on procède par double inclusion.
\begin{outline}
	\1 \textbf{Montrons l'inclusion $\ker(P(u)) + \ker(Q(u)) \subset \ker((PQ)(u))$ :}
	\2 Soit $x \in \ker(P(u))$ et $y \in \ker(Q(u))$. Alors $P(u)(x) = 0$ et $Q(u)(y) = 0$.
	\2 Par commutativité des polynômes d'endomorphismes, on a :
	$$\begin{cases}
	(PQ)(u)(x) = (Q(u)\circ P(u))(x) = Q(u)(P(u)(x)) = Q(u)(0) = 0 \\
	(PQ)(u)(y) = (P(u)\circ Q(u))(y) = P(u)(Q(u)(y)) = P(u)(0) = 0
	\end{cases}$$
	\2 Cela implique que :
	$$\ker(P(u)) \subset \ker((PQ)(u)) \quad \text{et} \quad \ker(Q(u)) \subset \ker((PQ)(u))$$
	\2 Par structure de sous-espace vectoriel, leur somme est également incluse dans le sous-espace de destination :
	$$\ker(P(u)) + \ker(Q(u)) \subset \ker((PQ)(u))$$
	\1 \textbf{Montrons l'inclusion $\ker((PQ)(u)) \subset \ker(P(u)) + \ker(Q(u))$ :}
	\2 Soit $x \in \ker((PQ)(u))$. Alors $(PQ)(u)(x) = 0$.
	\2 En utilisant l'identité d'endomorphismes, on a :
	$$x = \mathrm{Id}_E(x) = U(u)P(u)(x) + V(u)Q(u)(x)$$
	\2 Posons $a = V(u)Q(u)(x)$ et $b = U(u)P(u)(x)$. Alors $x = a + b$.
	\2 Il reste à montrer que $a \in \ker(P(u))$ et $b \in \ker(Q(u))$.
	\3 Pour $a$ :
	$$P(u)(a) = P(u)(V(u)Q(u)(x)) = V(u)P(u)Q(u)(x) = V(u)(PQ)(u)(x) = V(u)(0) = 0$$
	\3 Pour $b$ :
	$$Q(u)(b) = Q(u)(U(u)P(u)(x)) = U(u)Q(u)P(u)(x) = U(u)(PQ)(u)(x) = U(u)(0) = 0$$
	\2 Ainsi, $a \in \ker(P(u))$ et $b \in \ker(Q(u))$, ce qui implique que $x = a + b \in \ker(P(u)) + \ker(Q(u))$.
	\2 Par conséquent, on a montré que :
	$$\ker((PQ)(u)) \subset \ker(P(u)) + \ker(Q(u))$$
	\2 Par double inclusion, on conclut que :
	$$\ker((PQ)(u)) = \ker(P(u)) + \ker(Q(u))$$
	\1 Il reste à montrer que la somme est directe, c'est-à-dire que $\ker(P(u)) \cap \ker(Q(u)) = \{0_E\}$.
	\2 Soit $x \in \ker(P(u)) \cap \ker(Q(u))$. Alors $P(u)(x) = 0$ et $Q(u)(x) = 0$.
	\2 En utilisant l'identité d'endomorphismes, on a :
	$$x = U(u)P(u)(x) + V(u)Q(u)(x) = U(u)(0) + V(u)(0) = 0_E$$
	\2 Ainsi, $\ker(P(u)) \cap \ker(Q(u)) = \{0_E\}$, ce qui montre que la somme est directe.
	\1 Par conséquent, on a bien :
	$$\ker((PQ)(u)) = \ker(P(u)) \oplus \ker(Q(u))$$
\end{outline}

\clearpage

\section{Chapitre XIV-C --- Matrices}

\begin{tcolorbox}[instructionstyle]
	Montrer que pour tout $\mathbb K$-espaces vectoriels $E$ et $F$ de dimensions finies $p$ et $n$ respectivement, il existe un isomorphisme canonique entre $\mathcal L(E,F)$ et $\mathcal M_{n,p}(\mathbb K)$.
\end{tcolorbox}

\begin{rappel}
	Une application linéaire de $E$ vers $F$ est caractérisée par les images de la base de $E$ dans $F$.
\end{rappel}

\begin{tcolorbox}[methodstyle]
	On pose $\psi : \mathcal L(E,F) \to \mathcal M_{n,p}(\mathbb K)$ l'application qui à chaque application linéaire $f$ associe sa matrice dans des bases fixées de $E$ et $F$, i.e. $\psi(f) = \mathrm{Mat}(f,B,C)$. On montre que $\psi$ est bien définie, injective et surjective, ce qui établit l'isomorphisme entre les deux espaces vectoriels.
	
	Pour montrer que $\psi$ est surjective, on prend une matrice $A \in \mathcal M_{n,p}(\mathbb K)$ et on construit une application linéaire $f$ telle que $\mathrm{Mat}(f,B,C) = A$, en faisant en sorte qu'elle associe à chaque vecteur de la base de $B = (e_1, \dots, e_p)$ de $E$ une combinaison linéaire des vecteurs de la base $C = (f_1, \dots, f_n)$ de $F$ avec les coefficients donnés par les colonnes de la matrice $A$, i.e. $f(e_j) = \sum_{i=1}^{n} a_{ij} f_i$ pour chaque $j = 1, \dots, p$. Ainsi, on obtient une application linéaire $f$ dont la matrice dans les bases $B$ et $C$ est exactement $A$, ce qui prouve la surjectivité de $\psi$.
\end{tcolorbox}
...
\begin{itemize}
	...
	\item On a $\mathrm{Mat}(f,B,C) = \psi(f) \in \mathcal M_{n,p}(\mathbb{K})$.
	\item Donc $\psi$ est définie.
\end{itemize}
\textbf{$\psi$ est une application linéaire :}
\begin{outline}
\1 On pose $N = \mathrm{Mat}(f,B,C) = (a_{i,j})_{\substack{1 \le i \le n\\ 1 \le j \le p}}$.
\2 On a donc $N = \left(\begin{matrix} f(e_1) & \cdots & f(e_p)\\
a_{1,1} & \cdots & a_{1,p} \\
\vdots & & \vdots \\
a_{n,1} & \cdots & a_{n,p}
\end{matrix}\right)$.
\1 On pose $M = \mathrm{Mat}(g,B,C) = (b_{i,j})_{\substack{1 \le i \le n\\ 1 \le j \le p}}$.
...
\end{align*}
\1 Cela implique que $\underbrace{\mathrm{Mat}(f+\lambda g,B,C)}_{=\psi(f+\lambda g)}= \underbrace{N}_{\psi(g)} + \lambda \underbrace{M}_{\psi(g)}$.
\1 Donc $\psi \in \mathcal L(\mathcal L(E,F), \mathcal M_{n,p}(\mathbb{K}))$.
\end{outline}
\textbf{$\psi$ est injective :}
\begin{outline}
\1 Soit $f \in \ker(\psi)$. Donc $\psi(f) = 0_{n,p}$.
\1 Donc $\forall 1 \le j \le p, f(e_j) = 0_F$.
\1 Or $f(E) = \mathrm{Vect}(f(B)) = \{0_F\}$.
...
\begin{outline}
	\1 Soit $A = (a_{i,j})_{\substack{1 \le i \le n\\ 1 \le j \le p}} \in \mathcal M_{n,p}(\mathrm{K})$.
	...
	\1 Or $\psi(f) = \mathrm{Mat}(f,B,C) = A$.
	\1 Donc $\psi$ est surjective !
\end{outline}
On a montré que $\psi$ est un isomorphisme de $\mathbb{K}$-ev. entre $\mathcal L(E,F)$ et $\mathcal M_{n,p}(\mathbb{K})$.

\clearpage 

\begin{tcolorbox}[instructionstyle]
Soient $E, F, G$ trois $\mathbb{K}$-ev de dimensions finies $q, p, n$, de bases respectives $B = (e_1,\dots,e_q)$, $C = (c_1,\dots,c_p)$ et $D = (d_1,\dots,d_n)$.
Soient $f \in \mathcal L(E,F)$ et $g \in \mathcal L(F,G)$ de matrices $A_1 = \mathrm{Mat}(f,B,C) = (b_{i,j})$ et $A_2 = \mathrm{Mat}(g,C,D) = (a_{i,j})$.

Montrer que $\mathrm{Mat}(g \circ f, B, D) = (\alpha_{i,j})_{\substack{1 \le i \le n \\ 1 \le j \le q}}$ où $\alpha_{i,j} = \sum_{k=1}^p a_{i,k} b_{k,j}$.
\end{tcolorbox}

\begin{rappel}
Par définition de la matrice d'une application linéaire :
$$\forall j \in \llbracket 1,q \rrbracket, \quad f(e_j) = \sum_{k=1}^p b_{k,j} c_k \quad \text{et} \quad \forall k \in \llbracket 1,p \rrbracket, \quad g(c_k) = \sum_{i=1}^n a_{i,k} d_i$$
L'unicité de la décomposition dans une base permet l'identification des coefficients d'un vecteur.
\end{rappel}

\begin{tcolorbox}[methodstyle]
On calcule $(g \circ f)(e_j)$ de deux manières : par sa définition matricielle directe, et en développant $g(f(e_j))$ par linéarité. L'interversion des sommes finies permet d'isoler le coefficient recherché par identification dans la base $D$.
\end{tcolorbox}

\vspace{0.4cm}
\textbf{Démonstration :}

Soit $j \in \llbracket 1,q \rrbracket$. On pose $\mathrm{Mat}(g \circ f, B, D) = (\alpha_{i,j})$. Par définition :
$$(g \circ f)(e_j) = \sum_{i=1}^n \alpha_{i,j} d_i \quad (\ast)$$

Par définition de la composée et par linéarité de $g$ :
$$(g \circ f)(e_j) = g\left(f(e_j)\right) = g\left(\sum_{k=1}^p b_{k,j} c_k\right) = \sum_{k=1}^p b_{k,j} g(c_k)$$

En injectant la décomposition de $g(c_k)$ dans la base $D$ :
$$(g \circ f)(e_j) = \sum_{k=1}^p b_{k,j} \left(\sum_{i=1}^n a_{i,k} d_i\right)$$

Les sommes étant finies, on permute l'ordre des sommations et on utilise la commutativité du corps $\mathbb{K}$ :
$$(g \circ f)(e_j) = \sum_{i=1}^n \left(\sum_{k=1}^p a_{i,k} b_{k,j}\right) d_i \quad (\ast\ast)$$

La famille $D$ étant une base de $G$, la décomposition de chaque vecteur y est unique. Par identification des coefficients entre $(\ast)$ et $(\ast\ast)$, on obtient pour tout $i \in \llbracket 1,n \rrbracket$ :
$$\alpha_{i,j} = \sum_{k=1}^p a_{i,k} b_{k,j}$$

Ce coefficient correspond par définition au terme général de la matrice produit $A_2 \times A_1$. Ainsi :
$$\mathrm{Mat}(g \circ f, B, D) = \mathrm{Mat}(g, C, D) \times \mathrm{Mat}(f, B, C)$$

\clearpage

\begin{tcolorbox}[instructionstyle]
	Montrer que $\forall A_1, A_2 \in \mathcal M_{n,p}(\mathbb K)$ :
	
	\begin{gather*}
		A_1 \text{ éq } A_2 \iff \exists f \in \mathcal L(E,F), \exists B_1,B_2 \text{ bases de } E, \exists C_1,C_2 \text{ bases de } F \text{ telles que }
		\\
		A_2 = \mathrm{Mat}(f,B_2,C_2) \text{ et } A_1 = \mathrm{Mat}(f,B_1,C_1)
	\end{gather*}
\end{tcolorbox}

\begin{tcolorbox}[methodstyle]
    On procède par double implication. Dans le sens indirect, on part de l'existence d'une même application linéaire $f$ exprimée dans deux couples de bases distincts. L'application de la formule de changement de base génère immédiatement la relation algébrique $A_2 = P_{C_2}^{C_1} A_1 P_{B_1}^{B_2}$. L'inversibilité intrinsèque des matrices de passage valide l'équivalence des matrices. Dans le sens direct, on part de la relation $A_2 = P A_1 Q$ avec $P$ et $Q$ inversibles. On introduit l'application linéaire $f$ canoniquement associée à $A_1$ dans les bases canoniques. L'inversibilité de $Q$ implique que ses colonnes forment la nouvelle base de départ $B_2$. L'inversibilité de $P^{-1}$ implique que ses colonnes forment la nouvelle base d'arrivée $C_2$, ce qui permet d'identifier $P$ à la matrice de passage $P_{C_2}^{C_1}$.
\end{tcolorbox}

On procède par double implication.

\paragraph{Sens indirect ($\Leftarrow$) : }
Supposons qu'il existe un couple d'espaces vectoriels $(E,F)$ de dimensions respectives $p$ et $n$, une application linéaire $f \in \mathcal{L}(E,F)$, ainsi que deux bases $B_1, B_2$ de $E$ et deux bases $C_1, C_2$ de $F$ telles que $A_1 = \mathrm{Mat}(f, B_1, C_1)$ et $A_2 = \mathrm{Mat}(f, B_2, C_2)$.

D'après la formule générale du changement de base pour une application linéaire, on a la relation :
\[
\mathrm{Mat}(f, B_2, C_2) = P_{C_2}^{C_1} \cdot \mathrm{Mat}(f, B_1, C_1) \cdot P_{B_1}^{B_2}
\]
Ce qui se réécrit avec les matrices données :
\[
A_2 = P_{C_2}^{C_1} \cdot A_1 \cdot P_{B_1}^{B_2}
\]
Puisque $P_{C_2}^{C_1} \in GL_n(\mathbb{K})$ et $P_{B_1}^{B_2} \in GL_p(\mathbb{K})$ sont des matrices inversibles (en tant que matrices de passage), il existe bien un couple de matrices inversibles $(P, Q)$ tel que $A_2 = P A_1 Q$. Par définition, $A_1$ et $A_2$ sont équivalentes ($A_1 \text{ éq } A_2$).

\paragraph{Sens direct ($\Rightarrow$) : }
Supposons que $A_1 \text{ éq } A_2$. Par définition de l'équivalence de matrices, il existe deux matrices inversibles $P \in GL_n(\mathbb{K})$ et $Q \in GL_p(\mathbb{K})$ telles que :
\[
A_2 = P \cdot A_1 \cdot Q
\]
Posons $E = \mathbb{K}^p$ et $F = \mathbb{K}^n$. On note $\mathcal{B}_{c_1}$ la base canonique de $\mathbb{K}^p$ et $\mathcal{B}_{c_2}$ la base canonique de $\mathbb{K}^n$.

Soit $f \in \mathcal{L}(\mathbb{K}^p, \mathbb{K}^n)$ l'application linéaire canoniquement associée à la matrice $A_1$. Par construction, on a :
\[
A_1 = \mathrm{Mat}(f, \mathcal{B}_{c_1}, \mathcal{B}_{c_2})
\]
L'objectif est de construire deux nouvelles bases, $\mathcal{B}$ pour $\mathbb{K}^p$ et $\mathcal{D}$ pour $\mathbb{K}^n$, de sorte que les matrices de passage associées s'identifient à $Q$ et $P$. La formule du changement de base impose l'identification suivante :
\[
Q = P_{\mathcal{B}_{c_1}}^{\mathcal{B}} \quad \text{et} \quad P = P_{\mathcal{D}}^{\mathcal{B}_{c_2}}
\]

\begin{enumerate}
    \item \textbf{Construction de la base de départ $\mathcal{B}$ :}
    Soient $C_1, \dots, C_p$ les vecteurs colonnes de la matrice $Q$. Comme $Q \in GL_p(\mathbb{K})$, ses colonnes forment une famille libre. Cette famille libre comporte $p$ vecteurs dans un espace de dimension $p$, elle constitue donc une base de $\mathbb{K}^p$, notée $\mathcal{B} = (C_1, \dots, C_p)$.
    
    Par définition, la matrice de passage de la base canonique $\mathcal{B}_{c_1}$ à la base $\mathcal{B}$ s'obtient en écrivant les vecteurs de $\mathcal{B}$ dans $\mathcal{B}_{c_1}$. On a donc immédiatement :
    \[
    Q = \mathrm{Mat}_{\mathcal{B}_{c_1}}(\mathcal{B}) = P_{\mathcal{B}_{c_1}}^{\mathcal{B}}
    \]

    \item \textbf{Construction de la base d'arrivée $\mathcal{D}$ :}
    L'identification exige $P = P_{\mathcal{D}}^{\mathcal{B}_{c_2}}$. Par propriété d'inversion des matrices de passage, cela équivaut à chercher $\mathcal{D}$ telle que :
    \[
    P^{-1} = P_{\mathcal{B}_{c_2}}^{\mathcal{D}}
    \]
    La matrice $P$ étant inversible, sa matrice inverse $P^{-1}$ appartient également à $GL_n(\mathbb{K})$. Soient $C'_1, \dots, C'_n$ les vecteurs colonnes de $P^{-1}$. Cette famille est libre et comporte $n$ vecteurs en dimension $n$ ; elle forme donc une base de $\mathbb{K}^n$, notée $\mathcal{D} = (C'_1, \dots, C'_n)$.
    
    Par définition, la matrice de passage de la base canonique $\mathcal{B}_{c_2}$ à la base $\mathcal{D}$ est constituée des colonnes de $P^{-1}$ :
    \[
    P^{-1} = \mathrm{Mat}_{\mathcal{B}_{c_2}}(\mathcal{D}) = P_{\mathcal{B}_{c_2}}^{\mathcal{D}}
    \]
    En inversant de nouveau cette relation, on obtient :
    \[
    P = \left( P_{\mathcal{B}_{c_2}}^{\mathcal{D}} \right)^{-1} = P_{\mathcal{D}}^{\mathcal{B}_{c_2}}
    \]
\end{enumerate}

En substituant les expressions obtenues dans la relation algébrique initiale, on trouve :
\[
A_2 = P_{\mathcal{D}}^{\mathcal{B}_{c_2}} \cdot \mathrm{Mat}(f, \mathcal{B}_{c_1}, \mathcal{B}_{c_2}) \cdot P_{\mathcal{B}_{c_1}}^{\mathcal{B}}
\]
Le membre de droite correspond précisément à l'expression de l'application $f$ lue dans les bases $\mathcal{B}$ et $\mathcal{D}$. Par conséquent :
\[
A_2 = \mathrm{Mat}(f, \mathcal{B}, \mathcal{D})
\]
Les matrices $A_1$ et $A_2$ représentent bien la même application linéaire dans des bases différentes.
...
\section{Chapitre XIV-D --- Déterminants}
...
\begin{rappel}
	Un morphisme de groupe est une application $f : (G, \cdot) \to (H, *)$ entre deux groupes $(G, \cdot)$ et $(H, *)$ telle que pour tous $x, y \in G$, on a :
	\[
	f(x \cdot y) = f(x) * f(y)
	\]
\end{rappel}
