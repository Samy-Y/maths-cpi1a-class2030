...
\begin{property}
    \begin{enumerate}
        \item $S_{n}(\Kset)$ et $A_{n}(\Kset)$ sont des sous-espaces vectoriels supplémentaires isomorphes de $\mathcal M_n(\Kset)$.
...
\begin{example}
    ...
    Donc $A \in \GL_2(\R)$ avec $A^{-1} = -\frac{1}{2}(-3A - 2I_2) = \frac{3}{2} A + I_2 = \left(\begin{matrix} \frac{1}{2} & -3 \\ \frac{9}{2} & 7 \end{matrix}\right)$.es
\end{example}
...
\begin{exercise}
    ...
    Et $n+1 \ne 0$ donc $A \in \GL_n(\R)$ avec $A^{-1} = -\frac{1}{n+1}(-(n+2)A + (n+1)I_n) = \frac{n+2}{n+1} A - I_n$.
\end{exercise}
...
\begin{property}
    ...
    \begin{enumerate}
        ...
        \item Soit $x \in E$. On pose $x_1 = \Mat_{B_1}(x)$ et $x_2 = \Mat_{B_2}(x)$. Alors $x_2 = P_{B_1}^{B_2} \times x_1$, i.e. $\Mat_{B_2}(x) = \mathrm{Pass}(B_1,B_2) \times \Mat_{B_1}(x)$.
    \end{enumerate}
\end{property}
...
\begin{example}
    ...
    $$\begin{cases}
    P_{B_C}^B = \left(\begin{matrix} 1 & 0 & 0 \\ 1 & 1 & 0 \\ 1 & 1 & 1 \end{matrix}\right)\\
    P_B^{B_C} = \left(\begin{matrix} 1 & 0 & 0 \\ -1 & 1 & 0 \\ 0 & -1 & 1 \end{matrix}\right)\\
    \Mat_B(Q) = P_B^{B_C} \times \Mat_{B_C}(Q) = \left(\begin{matrix} 1 & 0 & 0 \\ -1 & 1 & 0 \\ 0 & -1 & 1 \end{matrix}\right) \times \left(\begin{matrix} 1 \\ -2 \\ 1 \end{matrix}\right) = \left(\begin{matrix} 1 \\ -3 \\ 3 \end{matrix}\right)
    \end{cases}$$
\end{example}
...
\begin{property}
    ...
    Alors $A_2 = P_{C_1}^{C_2} \times A_1 \times P_{B_2}^{B_1}$.
...
\begin{consequence}
    ...
    Alors $[f]_{B_2} = P_{B_1}^{B_2} \times [f]_{B_1} \times P_{B_2}^{B_1}$.
\end{consequence}
...
\begin{proof}
    ...
    Donc $\Mat(\Id_E, B_1, B_3) = \Mat(\Id_E, B_2, B_3) \times \Mat(\Id_E, B_1, B_2)$.
